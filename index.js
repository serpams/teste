...
const BOTConnection = async () => {
  ...
  app.post(
    "/send-image",
    [body("jid").notEmpty(), upload.single("image")],
    async (req, res) => {
      const errors = validationResult(req).formatWith(({ msg }) => {
        return msg;
      });
      ...
      const buttonMessage = {
        image: readFileSync("./assets/cfc.jpg"),
        footerText: "Teste",
        headerType: 4,
      };
      BotSendMessage(jid, buttonMessage)
        .then((response) => {
          res.status(200).json({
            status: true,
            response: response,
  ...
  // create a route to recept image and send to whatsapp
};
